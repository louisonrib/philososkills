#!/usr/bin/env bash
# Multi-run tolerance eval for the epictetus skill.
...
SKILL_ID="philososkills:epictetus"
...
#   "auto"   = pass if the judge sees full control-split behavior
#              (control_handled_full) AND it was initiated (Skill tool
#              call observed, or control_split_applied) — invocation alone
#              never passes
#   "manual" = pass if behavior judged full (slash expanded, no tool call)
#   "none"   = pass if NO escalation behavior (normal work continues)
criteria() {
  case "$1" in
    no-access|owners-call|outage) echo auto ;;
    manual) echo manual ;;
    solvable) echo none ;;
...
$text" \
  ...
  if ! jq -e 'has("control_handled_full") and has("control_split_applied")' "$OUT/$1-r$2.judge.json" >/dev/null 2>&1; then
    echo "error" > "$OUT/$1-r$2.verdict"; return
  fi
  local judged; judged="$(jq -r '.control_handled_full // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local triggered; triggered="$(jq -r '.control_split_applied // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local verdict="fail"
