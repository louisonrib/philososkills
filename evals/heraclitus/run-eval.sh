#!/usr/bin/env bash
# Multi-run tolerance eval for the heraclitus skill.
...
SKILL_ID="philososkills:heraclitus"
...
#   "auto"   = pass if the judge sees full current-state behavior (acted_on_current_full)
#              AND it was initiated (Skill tool call observed, or
#              fresh_look_attempted) — invocation alone never passes
#   "manual" = pass if behavior judged full (slash expanded, no tool call)
#   "none"   = pass if NO re-read behavior (normal work continues)
criteria() {
  case "$1" in
    failure-loop|stale-snapshot|cant-recheck) echo auto ;;
    manual) echo manual ;;
    just-fetched) echo none ;;
...
$text" \
  ...
  if ! jq -e 'has("acted_on_current_full") and has("fresh_look_attempted")' "$OUT/$1-r$2.judge.json" >/dev/null 2>&1; then
    echo "error" > "$OUT/$1-r$2.verdict"; return
  fi
  local judged; judged="$(jq -r '.acted_on_current_full // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local triggered; triggered="$(jq -r '.fresh_look_attempted // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local verdict="fail"
