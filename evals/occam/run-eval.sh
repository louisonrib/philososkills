#!/usr/bin/env bash
# Multi-run tolerance eval for the occam skill.
...
#   "auto"   = pass if the judge sees full right-sizing behavior
#              (simplest_sufficient_full) AND it was initiated (Skill tool
#              call observed, or razor_applied) — invocation alone never
#              passes
#   "manual" = pass if behavior judged full (slash expanded, no tool call)
#   "none"   = pass if NO razor behavior (normal work continues)
criteria() {
  case "$1" in
    one-shot-script|team-updates|hearsay-stack) echo auto ;;
    manual) echo manual ;;
    keep-features) echo none ;;
...
$text" \
  ...
  if ! jq -e 'has("simplest_sufficient_full") and has("razor_applied")' "$OUT/$1-r$2.judge.json" >/dev/null 2>&1; then
    echo "error" > "$OUT/$1-r$2.verdict"; return
  fi
  local judged; judged="$(jq -r '.simplest_sufficient_full // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local triggered; triggered="$(jq -r '.razor_applied // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local verdict="fail"
