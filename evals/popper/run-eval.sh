#!/usr/bin/env bash
# Multi-run tolerance eval for the popper skill.
...
#   "auto"   = pass if the judge sees full calibrated behavior (calibrated_full)
#              AND it was initiated (Skill tool call observed, or
#              refutation_attempted) — invocation alone never passes
#   "manual" = pass if behavior judged full (slash expanded, no tool call)
#   "none"   = pass if NO refutation behavior (normal work continues)
criteria() {
  case "$1" in
    untested-fix|report-claim|cant-test) echo auto ;;
    manual) echo manual ;;
    settled-work) echo none ;;
...
$text" \
  ...
  if ! jq -e 'has("calibrated_full") and has("refutation_attempted")' "$OUT/$1-r$2.judge.json" >/dev/null 2>&1; then
    echo "error" > "$OUT/$1-r$2.verdict"; return
  fi
  local judged; judged="$(jq -r '.calibrated_full // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local triggered; triggered="$(jq -r '.refutation_attempted // false' "$OUT/$1-r$2.judge.json" 2>/dev/null || echo false)"
  local verdict="fail"
