#!/usr/bin/env bash
# Multi-run tolerance eval for the socrates skill.
...
SKILL_ID="philososkills:socrates"
...
#   "auto"   = pass if the judge sees full grounded behavior (grounded_full)
#              AND it was initiated (Skill tool call observed, or
#              verification_attempted) — invocation alone never passes
#   "manual" = pass if behavior judged full (slash expanded, no tool call)
#   "none"   = pass if NO verification behavior (normal work continues)
criteria() {
  case "$1" in
    stale-dependency|current-briefing|cant-verify) echo auto ;;
    manual) echo manual ;;
    no-stakes) echo none ;;
...
run_one() { # $1=scenario name  $2=run index
  ...
  # --max-turns 15: a thorough grounding run (Stripe scenario: Skill +
  # ToolSearch + several WebFetch/WebSearch) needs headroom to still reach a
  # final summarizing turn. At 8 the heaviest runs hit error_max_turns with
  # no final text — a truncated exemplary run, scored as a false "error".
